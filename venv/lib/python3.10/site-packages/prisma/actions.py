            ...
            data=[
                {
                    # data to create a Job record
                    'title': 'bbbghgbadh',
                    'department': 'bbhcgagaic',
                    'location': 'ddaabegbb',
                    'employmentType': enums.EmploymentType.FULL_TIME,
                    'salaryMin': 1768156112,
                    'salaryMax': 71628745,
                    'salaryPeriod': enums.SalaryPeriod.yearly,
                    'description': 'ffhgghde',
                    'languages': Json({'ibcadcejf': True}),
                    'userId': 'bdcdfgccdg',
                },
                {
                    # data to create a Job record
                    'title': 'edhjgdfh',
                    'department': 'bdeffdadda',
                    'location': 'bjgfdihchf',
                    'employmentType': enums.EmploymentType.FULL_TIME,
                    'salaryMin': 804873448,
                    'salaryMax': 1566496513,
                    'salaryPeriod': enums.SalaryPeriod.yearly,
                    'description': 'ifaaaedja',
                    'languages': Json({'cbajdjjabf': True}),
                    'userId': 'bcicggedea',
                },
            ],
            skip_duplicates=True,
...
class CandidateActions(Generic[_PrismaModelT]):
    ...
    async def find_many(
        ...
        # find the first 5 Candidate records ordered by the resumeId field
    ...
    async def find_first(
        ...
        -------
        ```py
        # find the second Candidate record ordered by the resumeName field
    ...
    async def find_first_or_raise(
        ...
        -------
        ```py
        # find the second Candidate record ordered by the name field
    ...
    async def group_by(
        ...
        *,
        ...
        -------
        ```py
        # group Candidate records by personalInfo values
        # and count how many records are in each group
            ...
            data=[
                {
                    # data to create a Application record
                    'jobId': 'ibbjaacbi',
                    'candidateId': 'djgacbcch',
                    'userId': 'geeeegace',
                },
                {
                    # data to create a Application record
                    'jobId': 'bbgdigchd',
                    'candidateId': 'dajcifgdi',
                    'userId': 'ccedhdbj',
                },
            ],
            skip_duplicates=True,
    ...
    async def create(
        ...
        interview = await Interview.prisma().create(
            data={
                ...
                'candidateId': 'cchghigae',
                'applicationId': 'ecdjjjhab',
                'scheduledById': 'biachfede',
                'jobId': 'fhgaibff',
        ...
        *,
        ...
        total = await Interview.prisma().create_many(
            data=[
                {
                    # data to create a Interview record
                    'candidateId': 'cadajbcbca',
                    'applicationId': 'bjheigfcdd',
                    'scheduledById': 'bjejigcdcg',
                    'jobId': 'bifiiibcah',
                    'type': enums.InterviewType.PHONE,
                    'scheduledAt': datetime.datetime.utcnow(),
                },
                {
                    # data to create a Interview record
                    'candidateId': 'dbjibjdaa',
                    'applicationId': 'dgijbdiaf',
                    'scheduledById': 'begfaigba',
                    'jobId': 'bdjiafcgjb',
                    'type': enums.InterviewType.PHONE,
                    'scheduledAt': datetime.datetime.utcnow(),
                },
    ...
    async def create(
        ...
        interviewresult = await InterviewResult.prisma().create(
            data={
                ...
                'interviewId': 'caffafcheh',
                'candidateId': 'fjjbegge',
                'applicationId': 'bdiifhbieb',
                'jobId': 'cdcaejhgg',
                'evaluatedCount': 918965159,
                'totalQuestions': 665143380,
                'averageFactualAccuracy': 39949309.190115,
                'averageCompleteness': 1627519811.109422,
                'averageRelevance': 356020985.163833,
                'averageCoherence': 1891567775.107277,
                'averageScore': 1879328181.157791,
                'passStatus': 'faehcjfdb',
                'summaryResult': 'bbaiefbee',
                'knowledgeLevel': 'bdaacgjbaf',
            },
        )
        ```
        """
        ...
        *,
        ...
        total = await InterviewResult.prisma().create_many(
            data=[
                {
                    # data to create a InterviewResult record
                    'interviewId': 'biibaighec',
                    'candidateId': 'baicdfeidj',
                    'applicationId': 'befgiciadg',
                    'jobId': 'cbcehahedh',
                    'evaluatedCount': 1298780834,
                    'totalQuestions': 1065892659,
                    'averageFactualAccuracy': 1266478832.192302,
                    'averageCompleteness': 95844078.189591,
                    'averageRelevance': 232393244.111693,
                    'averageCoherence': 1243026426.2135,
                    'averageScore': 831237128.102464,
                    'passStatus': 'ebedeihec',
                    'summaryResult': 'bajagjdfbb',
                    'knowledgeLevel': 'bggedbjggi',
                },
                {
                    # data to create a InterviewResult record
                    'interviewId': 'hgbafifcf',
                    'candidateId': 'bejiecfecg',
                    'applicationId': 'bjgacaeagh',
                    'jobId': 'beeaihbefg',
                    'evaluatedCount': 1515643353,
                    'totalQuestions': 916748190,
                    'averageFactualAccuracy': 44909183.45732,
                    'averageCompleteness': 2034942613.107979,
                    'averageRelevance': 417171355.138452,
                    'averageCoherence': 274854673.56897,
                    'averageScore': 721617527.151116,
                    'passStatus': 'ibijjdeb',
                    'summaryResult': 'caehiccddi',
                    'knowledgeLevel': 'bgcahjbafj',
                },
            ],
            skip_duplicates=True,
        )
        ...
        *,
        ...
        total = await ChatHistory.prisma().create_many(
            data=[
                {
                    # data to create a ChatHistory record
                    'interviewId': 'bjchdacjfa',
                    'question': 'fcadcaihb',
                },
                {
                    # data to create a ChatHistory record
                    'interviewId': 'bbdcibggga',
                    'question': 'bagbibfheb',
                },
            ],
            skip_duplicates=True,
...
class EvaluationActions(Generic[_PrismaModelT]):
        ...
        *,
        ...
        total = await Evaluation.prisma().create_many(
            data=[
                {
                    # data to create a Evaluation record
                    'interviewId': 'iicaadeeb',
                    'question': 'bigjggdhac',
                    'answer': 'bfdbbbjhad',
                },
                {
                    # data to create a Evaluation record
                    'interviewId': 'bdibjhdici',
                    'question': 'bceigehcbd',
                    'answer': 'caehahihfe',
                },
            ],
    ...
    async def update_many(
        ...
        total = await Evaluation.prisma().update_many(
            data={
                'factualAccuracyExplanation': 'cbaagdieci'
            },
            where={}
        )
        ```
